...
class ADB(Adapter):
    ...
    def shell(self, extra_args):
        ...
        if isinstance(extra_args, str) or isinstance(extra_args, str):
            ...
            for i in range(len(extra_args)):
                extra_args[i] = extra_args[i].replace('%s', ' ')
    ...
    def __transform_point_by_orientation(self, xy, orientation_orig, orientation_dest):
        ...
        if orientation_orig != orientation_dest:
            #portrait PRIMARY
            # developer.android.com/reference/androidx/browser/trusted/ScreenOrientation
    ...
    def type(self, text):
        ...
        self.shell("am broadcast -a ADB_INPUT_TEXT --es msg %s" % encoded)
